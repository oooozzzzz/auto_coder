...
function base64ToBlob(base64: string, contentType: string = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'): Blob {
...
export function useDocumentGenerator() {
  ...
  const generateDocument = async (
    ...
    try {
      ...
      const templateFile = await docxTemplateService.getTemplateFile(template.id);
    ...
    try {
      ...
      if (contentType?.includes('application/json')) {
        ...
        if (!result.success || !result.documents) {
          throw new Error(result.error || 'Ошибка при массовой генерации документов');
        }
    ...
    try {
      ...
      for (let i = 0; i < totalRows; i++) {
        ...
        if (!response.ok) {
          try {
            const errorData = await response.json();
            throw new Error(errorData.error || `Ошибка при генерации документа ${i + 1}`);
          } catch {
            throw new Error(`Ошибка при генерации документа ${i + 1}: ${response.status}`);
          }
        }
